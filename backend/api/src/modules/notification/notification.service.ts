...
export class NotificationService {
  ...
  async sendEmail(to: string, subject: string, body: string, htmlOverride?: string) {
    try {
      ...
      await this.sendWebhook({
        ...
        html,
        audience,
        reason: "notification_requested",
        emails: {
          client: audience === "client" ? { target: "client", to, subject, text, html, from, replyTo } : undefined,
          operator: audience === "operator" ? { target: "operator", to, subject, text, html, from, replyTo } : undefined,
          both: audience === "unknown" ? { target: "both", to, subject, text, html, from, replyTo } : undefined
        }
      });
    } catch (err) {
  ...
  async sendWhatsApp(to: string, body: string) {
    try {
      await this.sendWebhook({ channel: "whatsapp", to, body, reason: "notification_requested" });
    } catch (err) {
