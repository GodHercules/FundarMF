...
export class CompletedService {
  ...
  async reopen(id: string, body: { reason: string; kanbanStage: string }, actor: Actor) {
    ...
    const process = await this.prisma.process.findFirst({ where: { id, tenantKey: actor.tenantKey ?? "default" } }); if (!process || process.status !== ProcessStatus.CONCLUIDO) throw new NotFoundException("Processo finalizado não encontrado.");
  ...
  async getFile(contractId: string, fileId: string, actor: Actor) { await this.ensureContract(contractId, actor); const file = await this.prisma.contractFile.findFirst({ where: { id: fileId, contractId, contract: { tenantKey: actor.tenantKey ?? "default" } }, include: { contract: { include: { process: true } } } }); if (!file) throw new NotFoundException("Arquivo não encontrado."); if (actor.role === "OPERADOR" && file.contract.process?.ownerId !== actor.userId) throw new ForbiddenException(); return file; }
