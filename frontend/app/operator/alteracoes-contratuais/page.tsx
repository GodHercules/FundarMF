...
export default function OperatorAlteracoesContratuaisPage() {
  ...
  const canCreate = Boolean(types.length > 0 && clientName.trim() && clientEmail.trim() && documentNumber.trim() && (!types.includes("outra") || otherDescription.trim()));
  ...
  async function create() {
    ...
    try {
      const body = { clientName: clientName.trim(), clientEmail: clientEmail.trim(), documentNumber: documentNumber.trim(), alterationTypes: types, ...(types.includes("outra") ? { otherDescription: otherDescription.trim() } : {}) };
      ...
      setClientName(""); setClientEmail(""); setDocumentNumber(""); setTypes([]); setOtherDescription("");
  ...
  return <main className="app-container flex min-h-screen flex-col gap-8 py-12"><WorkspaceNav role="operator" />
    ...
    <section className="rounded-2xl border border-ink/10 bg-white/70 p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <fieldset className="rounded-xl border border-ink/10 bg-white/70 p-3 md:col-span-2">
          <legend className="px-1 text-sm font-medium text-ink">Onde será feita a alteração</legend>
          <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {ALTERACAO_CONTRATUAL_OPERATOR_OPTIONS.map(([id, label]) => <label key={id} className="flex cursor-pointer items-center gap-2 rounded-lg border border-ink/10 px-3 py-2 text-sm text-ink transition hover:border-brass/50 has-[:checked]:border-brass has-[:checked]:bg-brass/10">
              <input type="checkbox" checked={types.includes(id)} onChange={() => toggleType(id)} aria-label={label} />
              <span>{label}</span>
            </label>)}
          </div>
          <p className="mt-2 text-xs text-slate">Selecione uma ou mais opções.</p>
        </fieldset>
        ...
        {types.includes("outra") && <input className="input md:col-span-2" value={otherDescription} onChange={(event) => setOtherDescription(event.target.value)} placeholder="Descreva a outra alteração" aria-label="Descrição da outra alteração" />}
      </div>
