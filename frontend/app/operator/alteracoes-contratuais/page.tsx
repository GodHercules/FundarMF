...
export default function OperatorAlteracoesContratuaisPage() {
  ...
  const canCreate = Boolean(type && clientName.trim() && clientEmail.trim() && documentNumber.trim());

  async function create() {
    if (!canCreate || saving) return;
    setSaving(true); setMessage(null);
    try {
      const body = { clientName: clientName.trim(), clientEmail: clientEmail.trim(), documentNumber: documentNumber.trim(), alterationTypes: [type] };
      ...
      setClientName(""); setClientEmail(""); setDocumentNumber("");
  ...
  return <main className="app-container flex min-h-screen flex-col gap-8 py-12"><WorkspaceNav role="operator" />
    ...
    <section className="rounded-2xl border border-ink/10 bg-white/70 p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <select className="input md:col-span-2" value={type} onChange={(event) => setType(event.target.value)} aria-label="Onde será feita a alteração"><option value="" disabled>Onde será feita a alteração</option>{ALTERACAO_CONTRATUAL_OPTIONS.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select>
