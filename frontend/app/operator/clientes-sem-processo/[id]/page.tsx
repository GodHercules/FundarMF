...
export default function LegacyClientContractsPage() {
  ...
  function open(contract: Contract) {
    ...
    setContent(JSON.stringify(contract.versions[0]?.content ?? { type: "doc", content: [] }, null, 2));
  ...
  async function save() {
    ...
    try {
      const saved = await api<{ version: number }>(`/completed-processes/contracts/${selected.id}`, { method: "PUT", body: JSON.stringify({ title, content: JSON.parse(content), expectedVersion: selected.currentVersion }) });
      setSelected((current) => current ? { ...current, title, currentVersion: saved.version, versions: [{ version: saved.version, content: JSON.parse(content) }] } : current);
      setClient((current) => current ? { ...current, contracts: current.contracts.map((item) => item.id === selected.id ? { ...item, title, currentVersion: saved.version, versions: [{ version: saved.version, content: JSON.parse(content) }] } : item) } : current);
      dirtyRef.current = false;
  ...
  return <main className="app-container flex min-h-screen flex-col gap-6 py-12">
    ...
    {selected && <Card className="p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Editando documento</h2><p className="mt-1 text-sm text-slate">O conteúdo é salvo como nova versão.</p></div><Button variant="accent" onClick={() => void save()}>Salvar agora</Button></div><Input className="mt-4" aria-label="Título do documento" value={title} onChange={(event) => { setTitle(event.target.value); dirtyRef.current = true; }} /><textarea aria-label="Conteúdo estruturado editável" value={content} onChange={(event) => { setContent(event.target.value); dirtyRef.current = true; }} className="mt-3 min-h-[360px] w-full rounded-xl border border-ink/20 bg-white p-4 font-mono text-sm" /><Button className="mt-3" variant="ghost" onClick={() => setSelected(null)}>Fechar editor</Button></Card>}
  </main>;
}
